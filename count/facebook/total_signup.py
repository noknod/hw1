...
DONE_FILES_FILE = '../data/total_signup_files_done.txt'
...
def main():
    ...
    for file_path in all_files:
        if file_path not in done_files:
            ...
            begin_date = date_file + timedelta(days=-1)
            ...
            previous_files.append(
                    'hdfs:///user/yuklyushkin/hw1/metrics/{0}/total_signup.txt'.format(begin_date),                    
                    #'hdfs:///user/yuklyushkin/hw1/metrics/{0}/new_facebook_users.txt'.format(str_date_file)
                    )
            prevfiles = ','.join(previous_file_dates)
            #print(prevfiles)

            date_file_path = '/user/yuklyushkin/hw1/metrics/{0}/signup.txt'.format(str_date_file)
            if len(prevfiles) != 0:
                prevfiles = ',' + prevfiles
            ...
            command = 'hdfs dfs -cp out/part-00000 hw1/metrics/{0}/total_signup.txt'.format(dir_path)
